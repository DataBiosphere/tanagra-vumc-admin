...
## The script assumes that it is being run from the project-level directory "tanagra-vumc-admin/".
if [ $(basename $PWD) != 'tanagra-vumc-admin' ]; then
  echo "Script must be run from project-level directory 'tanagra-vumc-admin/'"
  exit 1
fi
...
# used for running the service during local and GHA testing
# this SA is on the allowlist for Verily's dev and test deployments
